  ...
  return (
    <section className="relative flex min-h-screen items-center justify-center px-6 py-40">
      ...
      <motion.div
        ...
        className="glass-card relative z-10 w-full max-w-2xl rounded-3xl p-10 md:p-16"
      >
        <div className="font-display text-white/90">
          ...
          >
            ...
            <span className="font-script text-5xl text-glow md:text-6xl" style={{ color: "#fde68a" }}>
              Chinmay
            </span>
          </motion.div>
        </div>
    ...
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-40">
      <div className="relative z-10 flex flex-col items-center gap-10">
        ...
        >
          ...
          {/* star nodes along the strokes */}
          {[
            [40,30],[65,30],[90,30],[65,65],[65,100],
            [120,30],[140,30],[160,30],[140,65],[140,100],[120,100],[160,100],
            [190,30],[215,70],[240,30],[215,100],
            [270,100],[282,66],[295,30],[308,66],[320,100],
          ].map(([x, y], i) => (
            <motion.circle
              key={i}
              cx={x}
...
function Flower({ x, y, hue, msg }: { x: number; y: number; hue: number; msg: string }) {
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="glass-card absolute left-1/2 z-30 mt-4 w-56 -translate-x-1/2 rounded-2xl px-4 py-3 text-center font-display text-sm italic text-amber-50"
          >
            {msg}
          </motion.div>
        )}
        ...
        <AnimatePresence>
          {wished && (
            <motion.div
              ...
              className="mt-10 font-script text-4xl text-glow md:text-6xl"
              style={{ color: "#fef3c7" }}
            >
              Make one wish.
            </motion.div>
  ...
  return (
    <section className="relative flex min-h-[80vh] items-center justify-center overflow-hidden px-6 py-32">
      <motion.p
        ...
        viewport={{ once: true }}
        transition={{ duration: 1.4 }}
        className="absolute top-24 font-display italic text-white/70"
      >
        Careful… something delicate is coming.
      </motion.p>
...
function Butterfly() {
  return (
    <svg viewBox="-60 -40 120 80" className="h-full w-full" style={{ filter: "drop-shadow(0 0 12px rgba(244,114,182,0.7))" }}>
      <defs>
...
function MoonScene() {
  ...
  return (
    <section ref={ref} className="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-40">
  ...
  return (
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-40">
      <div className="relative flex flex-col items-center gap-12">
        ...
        <svg viewBox="-24 -24 48 44" className="h-[340px] w-[340px] pulse-heart">
          {pts.map((p, i) => (
            <motion.circle
              key={i}
              cx={p.x}
        ...
        >
          ...
          <div className="mt-6 flex flex-col items-end">
            ...
            <span className="font-script text-4xl text-glow md:text-5xl" style={{ color: "#fde68a" }}>
              Chinmay ❤
            </span>
          </div>
        </motion.div>
...
function Ending() {
  ...
  return (
    <section className="relative flex min-h-[110vh] flex-col items-center justify-center gap-16 px-6 py-40">
      {lines.map((l, i) => (
        <motion.p
          key={i}
      ...
      <motion.div
        ...
        className="mt-10 text-xs tracking-[0.4em] text-white/40"
      >
        MADE WITH LOVE — CHINMAY, FOR TIYA
      </motion.div>
    </section>
  );
}
...
function Experience() {
  ...
  useEffect(() => {
    // subtle ambient piano (public CC0-ish tone via oscillator) — use Web Audio
    if (muted) return;
    ...
    const play = () => {
      ...
      notes.forEach((f, i) => {
        ...
        g.gain.linearRampToValueAtTime(0.05, now + i * 1.2 + 0.2);
        g.gain.linearRampToValueAtTime(0, now + i * 1.2 + 1.1);
        o.connect(g).connect(ctx.destination);
        o.start(now + i * 1.2);
        o.stop(now + i * 1.2 + 1.2);
      });
      setTimeout(play, notes.length * 1200);
    };
    play();
    return () => {
      stopped = true;
  ...
  return (
    <main className="aurora-bg relative min-h-screen overflow-x-hidden text-white">
      ...
      <div className="pointer-events-none fixed right-[6vw] top-[8vh] z-0 drift-slow opacity-90">
        <Moon size={180} />
      </div>
      {/* Floating fog */}
      <div className="pointer-events-none fixed inset-0 z-[5]" style={{
        background: "radial-gradient(1200px 400px at 50% 100%, rgba(192,132,252,0.15), transparent 60%), radial-gradient(1000px 300px at 20% 80%, rgba(125,211,252,0.12), transparent 60%)",
      }} />
      ...
      <button
        ...
        className="glass-card fixed right-6 top-6 z-[60] rounded-full px-4 py-2 text-xs tracking-widest text-white/80"
      >
